...
**Кнопка підсвітки** (`DISPLAY_BACKLIGHT_BUTTON_PIN`, 0=вимкнено):
опитується в тому самому циклі `run_forever()`, що й перемальовування
екрана — не окремим сервісом, бо `display.set_backlight()` можливий
лише через той самий об'єкт `st7789.ST7789`, що володіє BL-піном;
окремий процес конкурував би за той самий GPIO. Основний цикл тепер
опитує кнопку кожні `BUTTON_POLL_INTERVAL_SEC` (0.1с, для швидкого
відгуку), а перемальовує екран лише раз на `DISPLAY_REFRESH_SEC` —
відстежується окремим таймером `last_redraw`, не блокуючим сном.
`_button_edge()` — чиста edge-triggered функція (одне спрацювання на
натискання, не на кожен цикл опитування, поки утримується).

`app/gpio_utils.py` — спільна gpiod v1/v2-сумісна логіка читання
GPIO-входу, винесена з `shutdown_button.py` (той самий патерн
потрібен і для кнопки підсвітки) - `open_input_line(pin, consumer)`
