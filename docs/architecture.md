...
- `known_devices` — по одному рядку на dish_id: версії ПЗ dish/router
  ...
  перше знайомство з пристроєм/полем). `monitor.upsert_dish_and_notify()`/
  `upsert_router_and_notify()` НЕ вважають "🔄 оновлена" правильним
  формулюванням для БУДЬ-ЯКОЇ зміни — `_format_firmware_change_message()`
  (використовує `db.is_older_version()`, той самий толерантний
  компаратор, що для target-версій) розрізняє напрямок: "🔄 Прошивка X
  оновлена: X → Y" для forward-змін, "⏪ Прошивка X відкочена
  (можливо, SpaceX-side): X → Y" коли нова версія СТАРІША за попередню
  — знайдено на реальному сценарії користувача (SpaceX інколи відкочує
  прошивку глобально; ручний reboot може "спіймати" момент, коли
  команда відкату вже чекала), без розрізнення напрямку "🔄 оновлена"
  для факту, що насправді downgrade, вводило б в оману.
  `NOTIFY_FIRMWARE_ROLLBACK=0` (дефолт `1`) вимикає саме "⏪
  відкочена"-варіант окремо від "🔄 оновлена" (той лишається завжди
  активним) — `_format_firmware_change_message()` повертає `None`
  замість тексту, обидва виклики (`upsert_dish_and_notify()`/
  `upsert_router_and_notify()`) перевіряють на `None` перед `notify_
  fn(...)`. Спільний для dish і router (та сама функція).
...
(`Watchdog._maybe_scheduled_reboot()`) — незалежно від реальних збоїв
опитування (деякі користувачі практикують періодичний reboot для
профілактики). `reboot_dish()` — єдина доступна reboot-команда
(окремої router-reboot немає), перезавантажує ВЕСЬ фізичний Mini
(dish+router — той самий пристрій, той самий hardware/живлення).
`self.last_scheduled_reboot_ts` — ОКРЕМИЙ атрибут (не `last_reboot_
ts`, який ініціалізується `0.0` навмисно для "дозволити reboot одразу"
при auto-reboot-при-невдачах) — ІНІЦІАЛІЗУЄТЬСЯ `time.time()` при
`__init__`, інакше `now - 0.0` завжди величезне число, спричиняючи
негайний плановий reboot при КОЖНОМУ старті сервісу. `should_
scheduled_reboot()` (чиста функція) порівнює з `SCHEDULED_REBOOT_
INTERVAL_HOURS` (дефолт 24г). Поважає `MIN_REBOOT_INTERVAL_SEC` через
СПІЛЬНИЙ `last_reboot_ts` (з auto-reboot-при-невдачах) — якщо reboot
ВЖЕ стався нещодавно з ІНШОЇ причини, плановий природно
"відкладається", не подвоюється. `SCHEDULED_REBOOT_ENABLED` — дефолт
`0` (opt-in, свідомий вибір користувача — потенційно розриває
з'єднання на кілька хвилин).
