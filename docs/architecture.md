...
- `scripts/install.sh` — детектує install vs update режим
  ...
  після перевстановлення. Наприкінці — опційний інтерактивний блок
  налаштування статичних IP для eth0/wlan0 (з підтвердженням, дефолти
  редаговані), вимикає конфліктуючий `dhcpcd`.
