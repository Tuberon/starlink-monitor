...
**Накладання підписів кількох серій** — знайдено користувачем на
реальному скріншоті `/stats` (2 серії в одному графіку, напр. ping+
drop% чи throughput down/up): за замовчуванням `fillText()` малює
текст від `textBaseline='alphabetic'` (baseline не враховує
descenders українських літер типу "р"/"ц") — при кроці 12px і 10px
шрифті це могло візуально зливати два рядки підписів в один
нечитабельний блок. Виправлено явним `ctx.textBaseline = 'top'`
(текст малюється від верхньої межі, передбачувано) і збільшеним
кроком (14px, не 12px). Перевірено живим тестом — реальний виклик
`fillText()` для 2 серій, підтверджений `y`-крок рівно 14px.

**Лінія графіка перекриває текст підпису** — інша проблема, знайдена
користувачем на іншому реальному скріншоті: підписи не накладались
один на одного, АЛЕ сама лінія графіка проходила крізь текст, коли
дані саме в цій точці близькі до максимуму (лінія природно йде вгору
до того самого top-right кута, де розміщені підписи). Виправлено
напівпрозорим фоном-прямокутником під кожним підписом (`ctx.
measureText()` для точної ширини, малюється ПЕРЕД текстом) — колір
фону читається ДИНАМІЧНО з CSS-змінної `--sky-900` через
`getComputedStyle()` (не хардкоджений), бо ця змінна має РІЗНІ
значення для темної/світлої теми (`_hexToRgba()` конвертує
hex→rgba з прозорістю 0.8). Перевірено живими тестами для обох тем
окремо — фон малюється до тексту, колір коректно відповідає
активній темі в обох випадках.

**Hover-tooltip** — наведення курсору на графік показує точні значення
кожної серії в цій точці + час (`fmtTime()`). `canvas._chartState`
(series, options, геометрія) зберігається на самому DOM-елементі
після кожного рендера — hover-обробник (`_attachChartHover()`,
приєднується ОДИН раз на canvas, ідемпотентно через `canvas.
_hoverAttached`-прапорець) читає цей стан при кожному русі миші,
переобчислює найближчий index за X-координатою курсору, і викликає
`drawLineChart()` повторно з `options.hoverIndex` — той самий
рендер-шлях, що звичайний, тому вертикальна guideline і крапки на
кожній серії гарантовано збігаються з реальним положенням лінії
(`scaleInfo` — per-series `{min, range, scaleMax}`, обчислені в тому
самому циклі малювання, перевикористовуються для координат крапок,
не дублюється percentile-логіка). Tooltip — floating `div`
(`.chart-tooltip`, `.panel{position:relative}` для коректного
позиціонування), створюється раз при першому hover, оновлюється
через `innerHTML`. `options.timestamps` (опційний, той самий формат,
що `series[i].data` — паралельний масив unix-секунд) — без нього
tooltip показує лише значення, без часової мітки. Перевірено живим
тестом: listeners приєднуються один раз (ідемпотентність підтверджена
повторним викликом `drawLineChart()`), `mousemove` коректно обчислює
index і показує реальні значення, `mouseleave` ховає tooltip.
