...
дефолти проєкту відповідають їй (`WIDTH=170`, `HEIGHT=320`). Проміжна
версія мала дефолти 320×170 (поміняні місцями) — це був **хибний
висновок**: на момент того тесту RST-баг (див. запис нижче) ще
маскував справжню причину початкового порожнього екрана, і swap
випадково "покращив" картинку лише тому, що GRAM-адресація з
командуванням 320 стовпців (понад реальні 170 фізичних) давала
частковий, не повний, шум — не тому, що swap був правильним рішенням.
Офіційна документація LCDWIKI прямо підтвердила: панель фізично
170×320, зафіксовано в `docs/decisions-log.md`. Піни DC/RST/BL
(дефолти 25/24/18) все одно залежать від фактичного підключення до
Pi — редагуються через `/settings`. За специфікацією виробника,
`BLK` вмикається HIGH-рівнем; якщо підсвітку підключено напряму до
3.3V (не через GPIO Pi) — постав `STARLINK_DISPLAY_BL_PIN=0` (не
керувати програмно). Потребує `dtparam=spi=on` (вимкнено за
...
`DISPLAY_OFFSET_LEFT`/`DISPLAY_OFFSET_TOP` (дефолт 0) — передаються
напряму в конструктор `st7789.ST7789(offset_left=, offset_top=)`;
типова потреба для дешевих ST7789-клонів, де видима область менша за
фізичний GRAM контролера, підбирається емпірично.

**`DISPLAY_ROTATION` (дефолт 0)**: бібліотека `st7789` (Pimoroni)
забороняє `90`/`270`, коли `width != height` — власне обмеження
бібліотеки, не апаратна вимога контролера (офіційна документація
виробника показує підтримку всіх 4 поворотів навіть для 170×320 через
власну STM32-прошивку з прямими MADCTL-командами). Для 170×320
валідні лише `0`/`180` у межах цієї бібліотеки.

Офіційний мануал LCDWIKI (SKU MSP1901) підтверджує SPI mode 0
(CPOL=0, CPHA=0), MSB-first — бібліотека `st7789` вже встановлює це
жорстко (`mode=0`, `lsbfirst=False`), жодних додаткових налаштувань
не потрібно. Максимальна частота SPI в мануалі не вказана —
`DISPLAY_SPI_SPEED_HZ` (дефолт 40МГц) свідомо консервативний для
типового підключення джампер-дротами; піднімай через `/settings`,
якщо монтаж якісний (короткі дроти/шлейф).
...
`run_forever()`, не окремим сервісом — `display.set_backlight()`
можливий лише через той самий об'єкт `st7789.ST7789`, що володіє
BL-піном; окремий процес конкурував би за той самий GPIO.
