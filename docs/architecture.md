...
інтернет через wlan0 і `ip route replace default ... metric` динамічно
підвищує/знижує пріоритет дефолтного маршруту wlan0 (50 ⟷ 9999)
залежно від результату. Коли wlan0 демотовано (metric 9999), eth0 з
його стабільним metric 1002 автоматично стає дефолтним для **всієї
системи** (apt, curl тощо), не лише для нашого коду. Специфічні
маршрути `192.168.100.0/24`/`192.168.1.0/24` (dish/router) не
зачіпаються — завжди пріоритетніші за будь-який дефолтний маршрут.

Єдиний root-сервіс проєкту (потребує `CAP_NET_ADMIN` для зміни
таблиці маршрутів) — `CapabilityBoundingSet` звужує навіть root лише
до `CAP_NET_ADMIN`+`CAP_NET_RAW`. `uninstall.sh` відновлює
нормальний metric wlan0 перед видаленням таймера, якщо він був
демотований на момент видалення.
