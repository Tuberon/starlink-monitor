...
дозволяє явно вказати ОЧІКУВАНУ версію (напр. побачив у changelog,
чекаєш саме її), і отримати "✅ Останнє оновлення тарілки/роутера
встановлено: версія X" саме коли встановлена версія їй відповідає.
`Watchdog._check_target_version_reached()` (`app/monitor.py`) —
порівнює `current_version` з `settings["dish_target_version"]`
(звичайний рядок, `db.get_setting`/`set_setting`, не окрема таблиця).
...
НОВИЙ target (наступне очікуване оновлення), без потреби explicit
...
`POST /api/target-versions` приймає нову версію лише якщо вона НЕ
старіша за вже відому (`_is_older_version()`, `webapp.py`) — база
для порівняння = максимум із (попередній target, якщо був; поточна
встановлена версія). Захист від випадкового відкату (описка, забутий
раніше введений новіший target). `_version_key()` — толерантний
компаратор для формату `YYYY.MM.DD.mrXXXXX.N` (не строгий semver):
дата-префікс домінує як найнадійніший індикатор "новіше", посегментне
порівняння як fallback, коли дата однакова чи відсутня (не падає на
нетиповому форматі — регулярка просто не збігається, `date_part =
(0,0,0)` для обох, порівняння йде повністю по сегментах). Часткова
відмова: якщо dish valid, а router — ні (чи навпаки), валідне поле
зберігається, невалідне відхиляється окремо з поясненням у
`message` (не "усе або нічого").
