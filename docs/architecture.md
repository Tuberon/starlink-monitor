...
`Watchdog`, приймають `notify_fn: Callable[[str], None]` замість
`self._notify`). `Watchdog`-методи (`_check_target_version_reached`
тощо) стали тонкими обгортками навколо цих функцій — зворотно сумісні
для існуючого коду/тестів. `webapp.py` `api_check_updates()` (ручна
кнопка "Перевірити оновлення") викликає ТІ САМІ module-level функції
напряму (webapp.py і monitor.py — окремі systemd-сервіси/процеси, не
можуть ділити інстанс `Watchdog`) — **знайдена й виправлена реальна
прогалина**: раніше ручна кнопка лише записувала статус у БД, БЕЗ
жодного сповіщення (ні "✅ target-версія", ні "🔄 прошивка оновлена"),
навіть коли версія якраз збігалась із target у момент натискання.
`dish_id` для router-виклику з `api_check_updates()` — якщо dish
