...
зберігає ПАРУ (яка саме версія збіглась + повний target-список) —
природно "скидається", коли користувач змінює список (наприклад
додає версію для іншої апаратної ревізії), без потреби explicit
очищати стан. `/api/target-versions` (GET/POST) — CRUD, GET також
повертає `dish_current`/`router_current` (з `get_latest_metric()`/
`get_router_status()`) для **першочергового введення**: `settings.js`
`loadTargetVersions()` показує поточну відому версію як стартове
...
`_version_key()` — толерантний компаратор для формату `YYYY.MM.DD.
mrXXXXX.N` (не строгий semver): дата-префікс домінує як найнадійніший
індикатор "новіше", посегментне порівняння як fallback, коли дата
однакова чи відсутня (не падає на нетиповому форматі — регулярка
просто не збігається, `date_part = (0,0,0)` для обох, порівняння йде
повністю по сегментах). Часткова відмова МІЖ полями (dish vs router,
...
`_check_target_version_reached()`, але notified-ключ (`both_targets_
notified`) зберігає ОБ'ЄДНАНИЙ рядок обох target одночасно — природно
"скидається", щойно користувач змінить БУДЬ-ЯКИЙ з двох (не лише
обидва разом). Не входить у backup/restore — internal dedup-стан
...
Легкі горизонтальні grid-лінії (25/50/75%) і числовий підпис max-
...
сплющив би лінію в майже плоску риску, ховаючи важливі коливання.

**Percentile-based outlier-resistant масштабування** — знайдено
користувачем повторно на реальному скріншоті ("мало інформативності"):
один різкий викид (напр. початковий сплеск throughput при підключенні)
використовуючи raw max для стелі шкали стискав усю реально цікаву
варіацію решти даних у тонку смужку біля низу графіка. `_percentile()`
+ порівняння `trueMax` з 95-м процентилем (`p95`): якщо `trueMax > p95
* 1.5` — стеля шкали (`scaleMax`) стає `p95`, не `trueMax` (той самий
підхід, що Grafana для "outlier-resistant" осі Y). Точки, що
перевищують стелю, малюються "притиснутими" (`Math.min(v, scaleMax)`)
до верху графіка, не спотворюючи решту шкали. Текстовий підпис "макс"
і далі показує РЕАЛЬНИЙ max (`trueMax`, не обрізаний) — інформаційна
точність не постраждала, лише сама лінія графіка. Перевірено живим
тестом: звичайні дані без викиду (напр. ping) використовують повну
висоту графіка як і раніше (логіка не спрацьовує даремно), дані з
викидом (реалістичний відтворений сценарій зі скріншоту) — розмах Y
для "нормальних" значень зріс із дуже малого до ~86% доступної
висоти.

`canvas{width:100%; ...}` в `static/style.css` — без явного `width`
(ні CSS, ні HTML-атрибут) canvas використовує браузерний default
(300px), тому графіки малювались лише в межах цих 300px, ігноруючи
реальну ширину панелі (знайдено користувачем на реальному скріншоті —
графік не розтягувався на всю ширину блока). JS-код (`getBoundingClientRect()`
