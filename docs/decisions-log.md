...
## eth0-fallback для Telegram не вирішував NameResolutionError

**Симптом**: реальні логи показали `_request_with_eth0_fallback`
спрацьовує (лог "пробую через eth0" є), але помилка повторюється
щоразу з тією самою `NameResolutionError` - fallback не допомагав.

**Причина**: `_SourceAddressAdapter` прив'язує сокет лише для самого
HTTP-з'єднання. DNS-резолвінг (`getaddrinfo()`) відбувається РАНІШЕ,
через системний резолвер (`/etc/resolv.conf`), який сам вибирає
інтерфейс за таблицею маршрутизації - і теж іде непрацюючим wlan0.
Прив'язка сокета для HTTP тут ні до чого не давала.

**Виправлення**: `_resolve_via_eth0()` - явний DNS-запит (A-запис,
`dnspython`, нові залежності `dnspython>=2.6.1`) з параметром
`source=eth0_ip`, обходить системний резолвер повністю. Отриманий IP
підставляється через тимчасову (потокобезпечну, `_dns_patch_lock`)
підміну `socket.getaddrinfo()` лише для конкретного hostname -
`_force_hostname_resolution()`. `_request_with_eth0_fallback()` тепер
триступеневий: (1) дефолтний маршрут, (2) eth0 socket bind (працює,
якщо DNS системою резолвиться, але сам wlan0 не пропускає TCP), (3)
явний DNS через eth0 + eth0 socket bind (працює навіть коли системний
DNS теж не резолвиться) - саме сценарій із реальних логів.

## eth0-fallback досі не працював: source_address не форсує інтерфейс

**Симптом**: реальні логи показали, що fallback спрацьовує (лог
"пробую через eth0" є), але й ручний DNS-запит через eth0
(`source=eth0_ip`) теж падав з тайм-аутом на обох DNS-серверах.

**Причина**: прив'язка сокета до IP-адреси (`source_address`/
dnspython's `source=`) прив'язує лише ЗАЯВЛЕНУ джерельну IP пакета.
Linux обирає ФІЗИЧНИЙ інтерфейс за таблицею маршрутизації на основі
адреси ПРИЗНАЧЕННЯ, повністю ігноруючи джерельну IP сокета (якщо не
налаштована окрема policy-based routing на рівні ОС). Тобто пакет із
заявленим джерелом `192.168.0.95` (eth0) усе одно фізично йшов через
`wlan0` (бо туди веде маршрут до `8.8.8.8`) і там же й губився.
...
Python: `_Eth0BoundAdapter` (urllib3 `socket_options`, не
`source_address`) для HTTP; `_resolve_via_eth0()` - DNS-запит через
сокет із явним `SO_BINDTODEVICE` (`dns.query.udp(..., sock=...)` -
власний сокет, не dnspython's вбудований resolver). Коли й системний
DNS не резолвиться - запит іде напряму на резолвлений IP з `Host`
заголовком і `verify=False` (SNI/hostname-matching неможливий без
підміни за IP; ланцюжок довіри сертифіката лишається валідним, лише
проверка імені пропускається - свідомий компроміс для вузького
фолбек-вікна).

## Баг: WAN-failover фактично ніколи не спрацьовував (ip route replace)

**Симптом**: лог `wan_failover_check.sh` щоразу (кожні ~20с) рапортував
"знижую пріоритет", хоча начебто вже мав це зробити раніше - виглядало
як нескінченний цикл "виправлення".

**Причина**: `ip route show default` на реальному Pi показав ДВА
default-маршрути через wlan0 одночасно (`metric 50` і `metric 9999`).
`ip route replace default ... metric X` шукає точний збіг ЗА metric
теж - якщо запису з таким metric ще не було, команда ДОДАЄ новий
запис, не замінюючи старий. Ядро завжди обирає маршрут із найменшим
metric серед усіх для того самого призначення - тобто `wlan0`
(`metric 50`, старий) і далі перемагав `eth0` (`metric 1002`) попри
лог про "успіх". WAN-failover не діяв за призначенням від самого
початку.

**Виправлення**: замість умовного `replace`, скрипт тепер явно
видаляє ВСІ default-маршрути на wlan0 (`while ip route del ... ; do :;
done`) і додає рівно один чистий запис з потрібним metric - самозцілює
вже накопичені дублікати. Fast-path (нічого не робити, якщо стан вже
чистий - рівно один запис з правильним metric) зберігає той самий
мінімальний churn таблиці маршрутів. Той самий баг і те саме
виправлення застосовано в `uninstall.sh` (відновлення metric=50 при
видаленні проєкту).
