...
# Фізична кнопка увімкнення/вимкнення підсвітки дисплея (pull-up,
# LOW = натиснуто, toggle на кожне натискання - не hold, як у
# кнопки виключення Pi). 0 = вимкнено (за замовчуванням). Обробляється
# в тому самому процесі, що й сам дисплей (display.py), бо керування
# підсвіткою можливе лише через той самий об'єкт, що володіє SPI/BL-
# піном - окремий сервіс для кнопки конкурував би за той самий GPIO.
DISPLAY_BACKLIGHT_BUTTON_PIN = int(os.environ.get("STARLINK_DISPLAY_BACKLIGHT_BUTTON_PIN", "0"))
