...
7. Reboot/shutdown Pi з веб-інтерфейсу або фізичної GPIO-кнопки,
   ...
   зовнішнього моніторингу, встановлюваний як PWA — деталі нижче.
...
перезапуску сервісів). Bot token у файлі відкритий — зберігай як
secret.
...
лише локальний зв'язок з dish/router), і динамічно знижує пріоритет
дефолтного маршруту `wlan0`, коли супутниковий канал Starlink
недоступний — `eth0` автоматично стає дефолтним для **всієї системи**
(apt, curl, будь-яка програма), не лише для нашого коду. Специфічні
маршрути до dish/router не зачіпаються (завжди пріоритетніші за
дефолтний, незалежно від metric). Встановлюється й вмикається
автоматично разом з рештою сервісів.
