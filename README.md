...
- Не записуються й не показуються взагалі: попередження "карта перешкод
  скинута" (dish) і стан роутера "помилка завантаження оновлення" (тимчасова
  хмарна помилка SpaceX, роутер повторює сам)
