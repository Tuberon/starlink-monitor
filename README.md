...
- Часті reboot (флап) групуються — `STARLINK_REBOOT_SPAM_THRESHOLD`/
  `_WINDOW_SEC`: після порогу подальші мовчки рахуються (без
  окремого повідомлення про початок групування), потім підсумок
  після затишшя
...
- Зміна прошивки — сповіщення лише для реального оновлення ("🔄
  оновлена"); відкат (SpaceX інколи відкочує проблемні білди
  глобально) навмисно не сповіщається — `known_devices` все одно
  оновлюється поточною версією мовчки
...
Кнопка "📨 Надіслати backup у Telegram" на `/settings` — надсилає
той самий останній файл негайно, вручну, незалежно від того, чи
періодична відправка увімкнена (`STARLINK_TELEGRAM_BACKUP_ENABLED`
стосується лише автоматичного розкладу). Не створює новий backup —
лише надсилає вже наявний найновіший.
...
**Окремий, застосунок-рівня fallback для Telegram** — `telegram_
notify.py` не покладається лише на системний `nmcli`-failover вище
(20с-цикл міг би відставати від потреби надіслати сповіщення
негайно): якщо звичайний запит через дефолтний маршрут провалюється
мережевою помилкою, наступна спроба явно прив'язується до `eth0`
(`SO_BINDTODEVICE`), а якщо навіть системний DNS через `eth0` ще не
відповідає — ручний DNS-запит напряму через `eth0` на публічні DNS-
сервери. Це працює незалежно від системного WAN-failover і
застосовується автоматично до всіх Telegram-запитів (сповіщення,
команди бота, відправка backup) без додаткового налаштування.
