...
**Обмеження журналу при дуже тривалій недоступності**: після
`STARLINK_MAX_LOGGED_FAILURES` (за замовчуванням 30) послідовних
невдалих опитувань watchdog-спроби reboot перестають писатися в
журнал/БД (сама спроба reboot і далі відбувається щоцикл) — інакше
багатогодинна відсутність WiFi Starlink засмічує журнал сотнями
записів з дедалі більшим лічильником без нової корисної інформації.
Один фінальний запис позначає момент припинення логування.
...
**Цикл оновлення ПЗ dish**: Telegram повідомляє про початок
завантаження оновлення, готовність до перезавантаження та успішне
завершення (повернення в звичайний режим з новою версією).

## Backup/restore налаштувань

На сторінці `/settings`:
- **"Завантажити backup"** — JSON з bot token, chat_id, auto-reboot,
  фразами підпису та перевизначеними параметрами моніторингу
  (`app/config.py` — лише ті, що реально відрізняються від
  значень за замовчуванням)
- **"Відновити з backup"** — застосовує раніше збережений файл;
  відновлені параметри моніторингу застосовуються після перезапуску
  сервісів (кнопка "Зберегти й перезапустити сервіси" в панелі
