...
7. Надає веб-інтерфейс (Flask + Chart.js): live-метрики Starlink і Pi,
   ...
   попереджень, журнал подій з можливістю очищення, кнопки ручного
   reboot dish і ручної перевірки стану оновлень.
...
11. Надає віддалений доступ до Pi через **ZeroTier VPN**, з
    приєднанням/від'єднанням від мережі прямо з веб-інтерфейсу — без
    потреби заходити на пристрій по SSH.
...
фразі на рядок, можна редагувати вручну.

## VPN (ZeroTier)

Дозволяє підключатись до Pi віддалено (напр. з телефону чи іншого ПК)
без прокидання портів чи публічної IP-адреси. Налаштовується в панелі
"VPN (ZeroTier)" на дашборді, під панеллю Telegram-сповіщень:

1. Створіть мережу на [my.zerotier.com](https://my.zerotier.com) (безкоштовний тариф), скопіюйте Network ID.
2. Вставте Network ID у поле на дашборді, натисніть "Приєднатись".
3. Підтвердіть новий пристрій у списку учасників мережі на my.zerotier.com
   (за замовчуванням ZeroTier вимагає ручного схвалення кожного нового
   члена мережі).
4. Встановіть ZeroTier на свій телефон/ПК, приєднайтесь до тієї ж
   мережі — Pi стане доступний за виданою ZeroTier IP-адресою (напр.
   можна відкрити дашборд напряму: `http://<zerotier-ip>:8080`).

ZeroTier встановлюється автоматично при першому запуску `install.sh`
(офіційний install-скрипт `install.zerotier.com`). Network ID
зберігається в локальній БД, а `join`/`leave` виконуються через
`zerotier-cli` з обмеженим sudo-правом (див. `scripts/install.sh`).
...
│   ├── zerotier.py            # керування ZeroTier VPN (join/leave/статус)
